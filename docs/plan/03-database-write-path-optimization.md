...
2026-09-15 已完成 PostgreSQL 热路径剖析和一轮优化。`pg_stat_statements` 证明主要问题是发布水位落后后，四实例恢复扫描反复回放正常消息，并与消息序号分配竞争同一 `conversations` 行。当前实现已经：

- 将运行时发布水位移到 `conversation_publish_watermarks` 独立表；
- 用 4 个有界 worker 并发写不同会话的合并水位；
- 只恢复第一条未发布消息已滞留 30 秒的会话；
- 水位写失败只重新排队水位更新，不立即重复读取和发布消息。

同机、同口径的 2,000 连接剖析中，对端 P99 从 11,395.74ms 降到 771.16ms，恢复消息读取从 58,051 次降为 0；120,000 条消息全部写入、ACK、回显及送达，乱序和重复为 0。这个结果证明改动有效，但带 SQL 统计的 P99 仍高于 200ms 验收线。
...
**已实施并验证（2026-09-15）：** 独立发布水位表、有界并发水位写和按首条未发布消息年龄恢复。曾尝试按水位行更新时间延迟恢复，但老会话收到新消息时会被立即误判，恢复读取增加到约 6.36 万次、P99 恶化到约 12.88 秒，已回退。后续不得用“会话或水位行存在时间”代替“未发布消息滞留时间”。

后续实施：

1. 为恢复扫描和水位重试增加指标，分别观察扫描会话数、消息滞留时间和水位写失败数。
2. 如需继续降低扫描开销，先用 `EXPLAIN (ANALYZE, BUFFERS)` 验证 `(conversation_id, seq)` 唯一索引的查找成本，再考虑领导者选举或分片扫描。
3. 不要恢复批量 `UPDATE ... FROM (VALUES ...)`；本机已经验证它会扩大锁持有范围。
