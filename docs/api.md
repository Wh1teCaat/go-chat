...
refresh token 采用轮换机制：每次刷新会吊销旧 refresh token 并签发新的。旧 token 再次使用（重放）会返回 401，客户端必须保存响应里的新 `refresh_token`。服务端用 Redis 保存有效 refresh token 的 allowlist；Redis 是启动必需依赖。轮换通过 Lua 原子执行，每次刷新重新获得 7 天有效期。
...
`online` 来自 WebSocket 连接状态。当前服务使用 Redis 共享在线状态。
