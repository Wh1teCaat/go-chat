...
// 覆盖多单体实例负载均衡、Redis Pub/Sub 跨实例推送、幂等去重、多端同步、
// 增量补拉、token 轮换与吊销。
...
// check 输出冒烟检查结果，并收集失败步骤供测试结束时汇总。
...
// fatal 输出失败步骤和错误后终止冒烟测试。
func fatal(name string, err error) {
	fmt.Printf("FATAL %s — %v\n", name, err)
	os.Exit(1)
}

// post 向测试服务发送 JSON POST 请求并解析统一响应。
...
// dialWS 使用认证令牌建立冒烟测试 WebSocket 客户端。
func dialWS(name, token string) *wsClient {
	...
	return &wsClient{conn: conn, name: name, upstream: resp.Header.Get("X-Upstream-Addr")}
}
...
func main() {
	...
	upstreams := map[string]struct{}{wsA1.upstream: {}, wsA2.upstream: {}, wsB.upstream: {}}
	delete(upstreams, "")
	check("websockets reach multiple backends", len(upstreams) >= 2,
		fmt.Sprintf("A1=%q A2=%q B=%q", wsA1.upstream, wsA2.upstream, wsB.upstream))
